...
function animalia.initialize_lasso(self)
    ...
    if self.lasso_origin then
        ...
        and minetest.get_item_group(minetest.get_node(self.lasso_origin).name, "fence") > 0 then
            local object = minetest.add_entity(self.lasso_origin, "hades_animalia:lasso_fence_ent")
...
function animalia.set_lasso_visual(self, target)
    ...
    local object = minetest.add_entity(pos, "hades_animalia:lasso_visual")
...
minetest.register_entity("hades_animalia:frog_tongue_visual", {
    hp_max = 1,
    physical = false,
    collisionbox = {0, 0, 0, 0, 0, 0},
    visual = "mesh",

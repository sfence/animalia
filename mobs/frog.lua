...
creatura.register_mob("hades_animalia:frog", {
    -- Stats
    max_health = 5,
	...
	utility_stack = {
		...
		{
			utility = "hades_animalia:aquatic_wander",
			step_delay = 0.25,
			get_score = function(self)
		...
		{
			utility = "hades_animalia:eat_bug",
			get_score = function(self)
		...
		{
			utility = "hades_animalia:run_to_pos",
			get_score = function(self)
				if self.in_liquid then return 0 end
				local pos = self.object:get_pos()
				if not pos then return end
...
creatura.register_spawn_egg("hades_animalia:frog", "67942e", "294811")

...
creatura.register_mob("hades_animalia:bat", {
    -- Stats
    max_health = 5,
    ...
    step_func = function(self)
		...
		if self:timer(random(3,4)) then
			...
			and random(16) < 2
			and self:get_utility() == "hades_animalia:fly_to_roost" then
				pos = {
					x = floor(pos.x + 0.5),
					y = floor(pos.y + 0.5),
					z = floor(pos.z + 0.5)
				...
				if is_node_walkable(minetest.get_node(pos).name) then
					if minetest.get_node(vec_raise(pos, 1)).name ~= "hades_animalia:guano" then
						minetest.set_node(vec_raise(pos, 1), {name = "hades_animalia:guano"})
					else
...
creatura.register_spawn_egg("hades_animalia:bat", "392517", "321b0b")
